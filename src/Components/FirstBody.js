...
const FirstBody = (props) => {
  ...
  return (
    ...
    <div id='like'>
        <header id='tim'>
          <div className='edit'>
            <button className='button' onClick={Up} value={score}><img src={rick} id='born' alt=''></img></button>
            <h1>{score}</h1>
            <button className='button' onClick={Down} value={score}><img style={{height:'1vh'}} src={morty} id='born' alt='' ></img></button>
          </div>
          <aside className='side'>
            <img src={tin} alt='' id='bay'></img>
            <h2>{ride.user.username}</h2>
            <p id ='till'> {ride.createdAt}</p>
            <button style={{width:'7vh'}} className='button src'><img src={rig} alt='' onClick={Tip}></img><span>reply</span></button>
          </aside>
